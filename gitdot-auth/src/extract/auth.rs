...
use gitdot_core::{dto::JwtClaims, error::AuthenticationError, util::auth::GITDOT_SERVER_ID};
...
fn authenticate(parts: &Parts, app_state: &AppState) -> Result<Principal, AuthenticationError> {
    let header = parts
        ...
        .ok_or(AuthenticationError::MissingAuthHeader)?;

    let jwt = header
        .strip_prefix("Bearer ")
        .ok_or(AuthenticationError::InvalidAuthHeaderFormat)?;
