...
pub async fn execute(state: ExecutorState) -> Result<()> {
    let (basin_name, stream_name) = crate::util::s2::parse_s2_uri(&state.task.s2_uri)?;

    let stream = state.s2.basin(basin_name).stream(stream_name);
    let producer = stream.producer(ProducerConfig::default());

    let mut child = Command::new("sh")
        .args(["-c", &state.task.command])
    ...
    if !status.success() {
        anyhow::bail!("Task {} exited with status {}", state.task.id, status);
    }
    Ok(())
}
