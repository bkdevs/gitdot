...
import type { RepositoryBlobsResource, RepositoryPathsResource } from "gitdot-api";
...
function flattenTree(
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    lines.push({ hasMoreSiblings, isLast, name, path: entry.path, isTree, isExpanded, depth });
    if (isExpanded) {
      lines.push(
        ...flattenTree(entry.path, paths, expandedPaths, [...hasMoreSiblings, !isLast], depth + 1),
      );
    }
  }
  return lines;
...
function flattenAll(
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const isTree = entry.path_type === "tree";
    const isLast = i === entries.length - 1;
    lines.push({ hasMoreSiblings, isLast, name, path: entry.path, isTree, isExpanded: false, depth });
    if (isTree && depth + 1 < maxDepth) {
      lines.push(
        ...flattenAll(entry.path, paths, [...hasMoreSiblings, !isLast], depth + 1, maxDepth),
      );
    }
  }
  return lines;
...
function TreeLinePrefix({
  hasMoreSiblings,
  isLast,
}: { hasMoreSiblings: boolean[]; isLast: boolean }) {
  return (
...
function TreeHeader({
  ...
  const under = paths.entries.filter((e) => e.path.startsWith(prefix) && e.path !== folderPath);
  ...
  return (
    <div className="font-mono text-sm h-6 flex items-center justify-between pl-3 pr-2 pt-2 shrink-0">
      <div className="flex items-center">
        ...
        <span>/</span>
        <Link href={`/${owner}/${repo}`} className="hover:underline">{repo}</Link>
        {folderPath && folderPath.split("/").map((seg, i, arr) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: stable path segments
          <span key={i}>
            <span>/</span>
            <Link href={`/${owner}/${repo}/${arr.slice(0, i + 1).join("/")}`} className="hover:underline">{seg}</Link>
          </span>
        ))}
      </div>
    ...
    <>
      {lines.map((line) =>
        line.isTree ? (
          ...
          >
            <TreeLinePrefix hasMoreSiblings={line.hasMoreSiblings} isLast={line.isLast} />
            <Link
            ...
            >
              {showAbsolutePath && (
                <span className="text-muted-foreground">{line.path.split("/").slice(0, -1).join("/")}/</span>
              )}
              {line.name}/
            </Link>
            <span className="text-xs text-muted-foreground ml-auto flex items-center">
              {paths.entries.filter((e) => e.path.startsWith(`${line.path}/`) && e.path_type === "blob").length} files
            </span>
          </div>
        ) : (
          <Link
          ...
          >
            ...
            {blobs && (() => {
              const blob = blobs.blobs.find((b) => b.type === "file" && b.path === line.path);
...
export function FolderViewer({ folderPath }: { folderPath?: string }) {
  ...
  useEffect(() => {
    ...
    db.getPaths().then((p) => {
      ...
      setExpandedPaths(
        new Set(topLevel.filter((e) => e.path_type === "tree").map((e) => e.path)),
      );
    });
    db.getBlobs().then((b) => {
      if (b) setBlobs(b);
  ...
  return (
    <div className="flex w-full h-full min-h-0 overflow-hidden">
      ...
      >
        <TreeHeader folderPath={folderPath ?? ""} paths={paths} owner={owner} repo={repo} />
        <TreeRows
          lines={lines}
          paths={paths}
          blobs={blobs}
      ...
      <div className="flex-1 min-w-0 overflow-auto scrollbar-thin">
        {preview?.kind === "file" && (
          <div className="text-sm px-2 py-1.5">
            {toJsxRuntime(preview.hast, { Fragment, jsx, jsxs }) as React.JSX.Element}
          </div>
        )}
        {preview?.kind === "folder" && (
          <div className="flex flex-col">
            <TreeHeader folderPath={preview.path} paths={paths} owner={owner} repo={repo} />
            <TreeRows
              lines={preview.lines}
              paths={paths}
              blobs={blobs}
